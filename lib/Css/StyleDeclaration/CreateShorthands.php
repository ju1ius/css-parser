...
class CreateShorthands
{
    ...
    public function createBackgroundShorthand()
    {
        ...
        if (!$oBgImageProperty && !$oBgColorProperty) {
            return;
        }
    ...
    public function createDimensionsShorthands()
    {
        ...
        foreach ($aExpansions as $sProperty => $sExpanded) {
            ...
            if ($iImportantCount && $iImportantCount !== 4) {
                continue;
            }
            ...
            foreach ($aPositions as $sPosition) {
                ...
                $aValues[$sPosition] = Cloner::clone($aPropertyValues[0]);
            }
    ...
    public function createFontShorthand()
    {
        ...
        foreach (['font-style', 'font-variant', 'font-weight'] as $sProperty) {
            ...
            if ($aValues[0] !== 'normal') {
                $oNewValueList->append(Cloner::clone($aValues[0]));
            }
        }
        ...
        if ($oLHProperty) {
            ...
            if ('normal' !== $aLHValues[0]) {
                $val = new PropertyValueList(
                    [
                        Cloner::clone($aFSValues[0]),
                        Cloner::clone($aLHValues[0]),
                    ],
                    '/'
                );
                $oNewValueList->append($val);
            } else {
                $oNewValueList->append(Cloner::clone($aFSValues[0]));
            }
        } else {
            $oNewValueList->append(Cloner::clone($aFSValues[0]));
        }
        ...
        $oNewValueList->append(Cloner::clone($oFFProperty->getValueList()));
        ...
        foreach ($aProperties as $sProperty) {
            ...
            foreach ($aProperties as $iPos => $oProperty) {
                ...
                foreach ($aValues as $mValue) {
                    $aNewValues[$sDest][] = Cloner::clone($mValue);
                }
            }
        }
        $iImportantCount = count($aNewValues['important']);
    ...
    private function _destructiveCleanup(array $aProperties, $sShorthand)
    {
        ...
        foreach ($this->styleDeclaration->getProperties($sShorthand) as $iPos => $oProperty) {
            if ($iPos !== $iLastExistingShorthandPosition) {
